...
def load_data():
    ...
    try:
        ...
        st.stop() # Interrompe a execução para evitar o loading infinito
        return None, None
...
# --- Ferramenta de Diagnóstico na Sidebar ---
with st.sidebar:
    st.header("🛠️ Diagnóstico de Arquivos")
    st.markdown("Isso ajuda a verificar quais arquivos sua aplicação consegue enxergar.")
    
    current_directory = os.getcwd()
    st.write(f"Diretório de trabalho atual: `{current_directory}`")
...
if df_raw is not None and model is not None:
    ...
    st.warning("Ocorreu um problema ao carregar os dados ou o modelo. Verifique a seção de diagnóstico na barra lateral.")
